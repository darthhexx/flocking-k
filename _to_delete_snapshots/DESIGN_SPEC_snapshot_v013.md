...
**Engineering design specification — version 0.13**  
**Status:** M9C is a powered held-out `PARTIAL-GO`; bounded M9A.7/M9B integration improves movement-inclusive utility without weakening measured topology safety, but the colluding raw-channel weakness remains unresolved and production is not authorized  
**Date:** 23 July 2026
...
| Should acceleration be added now? | Not yet. The next informative steps are decentralized allocation and a separate raw-channel trust repair. Acceleration remains a later isolated ablation. |
...
### 3.2 Out of scope for version 0.13
...
Acceleration can enter in two distinct places and must be tested separately after an M9C successor clears decentralized action coordination and the raw-channel trust boundary:
...
The added state can improve sustained-turn prediction but also weakens observability, increases tuning burden, and can create overconfident extrapolation. Implement it only after those M9C follow-ups, initially as a local-filter ablation with the same fixed behavioural recurrence and truth-blind output policy. Promotion requires held-out recovery or decision-loss gains, calibrated covariance, and a complexity/runtime case.
...
- pursue decentralized action ownership, message-limited planner state, conflict resolution, and asynchronous replanning as the next allocator milestone;
- repair the permissive raw-measurement channel in a separate adversarial-trust milestone; do not claim that physical VOI motion addresses its absolute utility failure;
...
- do not add acceleration or revisit adaptive momentum until allocator decentralization and raw-channel trust boundaries are tested.

The M6/M7 evidence remains useful within its tested envelope. M8 supplied concrete topology and allocator failures; M8.1 established identifiability; M9B showed that nonmyopic allocation can beat simple mode spreading when viewpoints and costs genuinely compete; M9A repaired the dynamic uncertainty pathology without meeting the original stable-transfer margin; M9A.5 localized information and policy headroom; M9A.6 exposed admission-censored tails; M9A.7 repaired those tails in a powered prospective test; and M9C transferred the movement-efficiency mechanism into dynamic topology without resolving the raw-channel risk. The platform remains insufficient for deployment: the assignment and residual models are simulator-specific, the permissive raw channel pays severe loss under the colluding-ramp challenge despite selecting safely, the M9C planner is only component-local rather than genuinely decentralized, its gain depends on an explicit movement-cost model, and every result remains simulation-bound.
