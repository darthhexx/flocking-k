...
**Engineering design specification — version 0.11**  
**Status:** M9A remains a held-out `PARTIAL-GO`; M9A.6 is a held-out `NO-GO` after closing most policy headroom but failing the observer-regret tail in failure+partition  
**Date:** 22 July 2026
...
The current research claim remains bounded: the system can select when aggregated support is decisive and safely defer under symmetric ambiguity in this simulation family. M8 showed that agent failure plus graph partition broke calibrated transfer. M9A repairs uncertainty bookkeeping, rejoin shock, partition-local output, and covariance tails, but it does not meet the declared requirement that every dynamic scenario stay within `0.10` decision loss of stable operation. M9A.5 found substantial same-information policy headroom plus one narrow component-information boundary. M9A.6 closes most of that headroom with a causal assignment posterior, improves loss in every exact-model topology scenario, and detects two declared model-mismatch challenges, but two admission-censored failure+partition tails make its held-out observer-regret gate fail. M9B separately shows that two-step receding-horizon value of information can reduce movement-inclusive loss in a static centralized competing-view assay. None of these results qualifies the combined stack for production, and M9B integration remains unauthorized.
...
| Can M9B enter the production stack now? | No. It is centralized, static, and assumes a known pose-conditioned likelihood. M9A.6 did not clear its held-out observer-regret gate, so production-stack integration remains unauthorized. |
...
| Should acceleration be added now? | No. Topology-aware uncertainty and information-to-decision coupling are earlier blockers. Acceleration remains a later isolated ablation. |
...
| Explicit mode retention plus truth-blind, costed deferral | Engineering synthesis related to multi-hypothesis tracking, active hypothesis testing, and selective decision making. | Novelty remains unestablished until a broader focused review; the held-out safety result may be claimed within the simulator. |
...
### 3.2 Out of scope for version 0.11

- collision-certified trajectory planning;
- adversaries that adapt strategically to the detector;
- network packet framing, retries, energy, and cryptographic identity;
...
- decentralized or topology-stressed execution of the M9B planner;
- acceleration-state production models.
...
10. for M9A.6, updates the causal assignment posterior, predictive mismatch residual, credible modes, and risk-priced output action;
...
| M9B — Decision-relevant active sensing | Pose-conditioned hypothesis evidence, competing-view tasks, asymmetric costs, multi-step VOI, and unchanged passive/current/oracle controls. | VOI significantly lowers held-out movement-inclusive loss versus both current allocator and passive control, without unsafe selection. | Complete: `M9B-GO` in static centralized assay; integration waits for M9A |
| M10 — Acceleration ablation | Constant-acceleration or IMM local filters and acceleration-aware motion. | Held-out gain beyond the realism-qualified system with calibrated uncertainty and justified complexity. | Future |
| M11 — External replay | Recorded or hardware-in-the-loop trajectories and communication traces. | Deterministic ingestion, declared ground-truth limits, and transfer beyond synthetic data. | Future |
...
M9A.4 remains `PARTIAL-GO`, M9A.6 remains `NO-GO`, and `m9b_integration_authorized` is false. The bounded next milestone is an admission-aware M9A.7 developed on new seeds; acceleration and M9B production-stack integration remain blocked.
...
The architecture should still be retained: M9A.6 materially reduces exact-model loss without reopening covariance or wrong-selection failures. It must not be production-qualified because its held-out observer-regret tail fails. The next bounded implementation is **M9A.7 admission-aware missingness**:
...
4. stress the new measurement channel with Byzantine, rejoin, and stale-data cases before allowing it to bypass any belief-sharing gate;
...
The old M9A.4 and M9A.6 gates must not be retrospectively weakened. M9A.7 must answer its admission-aware question in a new prospective protocol.
...
Items 1–5 are implemented and passed their frozen static-assay gates. Item 6 remains deliberately unimplemented because M9A.6 failed its held-out observer-regret gate. M9B integration must wait for an admission-aware successor to pass; more static synthetic planner tuning would not address the blocking evidence failure. When integration is eventually authorized, compare the two-step planner against unchanged M8 active/no-investigation and M9B round-robin controls and preserve separate accuracy, movement, and component-boundary reporting.

## 12. Future acceleration feature

Acceleration can enter in two distinct places and must be tested separately after M9 clears the topology and utility blockers:

- **Estimator acceleration:** extend local state to `[p_x, p_y, v_x, v_y, a_x, a_y]`, or use an interacting multiple-model filter containing constant-velocity and constant-acceleration models.
- **Behavioural acceleration:** constrain changes in sensor velocity and add acceleration alignment or jerk penalties.

The added state can improve sustained-turn prediction but also weakens observability, increases tuning burden, and can create overconfident extrapolation. Implement it only after M9 qualifies the topology-resilient baseline, initially as a local-filter ablation with the same fixed behavioural recurrence and truth-blind output policy. Promotion requires held-out recovery or decision-loss gains, calibrated covariance, and a complexity/runtime case.
...
The M8 held-out directory contains the complete suite configuration, 4,000 per-run summaries, scenario aggregates, paired effects, 1,000 verified trace hashes, the machine-readable verdict, and the Markdown report. The M8.1 directory separately contains its run and policy/dose summaries, paired effects, verified fingerprints, verdict, and report. M9B adds 2,500 run rows, scenario/policy summaries, component-level effects, 500 verified fingerprints, and its frozen decision. M9A adds 4,000 run rows, scenario summaries, ablation/transfer effects, 800 verified fingerprints, and a replay-stable partial verdict. M9A.5 adds 800 paired frozen-trajectory rows, observer/global transfer effects, within-scenario decomposition gaps, 800 replayed fingerprints, and a non-promotional diagnosis. M9A.6 adds 300 training and 1,000 held-out paired rows, exact/off-model scenario summaries, observer-regret effects, replay manifests, frozen-motion audits, machine-readable decisions, and generated reports.
...
- retain M9A.6 as evidence that assignment weighting and risk pricing remove most exact-model policy loss, but respect its held-out `NO-GO` and do not authorize integration;
- develop only the bounded M9A.7 split-trust/admission-aware missingness repair on entirely new seeds, with explicit regret-tail protection;
- retain the M9B two-step VOI allocator for possible post-M9A.7 integration because it beat passive, myopic, and current round-robin controls on held-out movement-inclusive loss;
- describe the M9B result as movement efficiency with equal safety, not better accuracy or faster resolution;
- do not add acceleration or revisit adaptive momentum while admission-aware missingness and the M9B decentralization/integration boundary remain open.

The M6/M7 evidence remains useful within its tested envelope. M8 supplied concrete topology and allocator failures; M8.1 established identifiability; M9B supplied the first held-out evidence that nonmyopic allocation can beat simple mode spreading when viewpoints and costs genuinely compete; M9A repaired the dynamic uncertainty pathology without meeting the strict transfer margin; M9A.5 localized information and policy headroom; and M9A.6 removed most policy loss while exposing admission-censored missingness in rare failure+partition tails. The platform remains insufficient for deployment: the assignment and residual models are simulator-specific, the residual predictive check conditions only on observed data, M9B is centralized and static, its gain depends on an explicit movement-cost model, and every result remains simulation-bound.
